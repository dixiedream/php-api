...
use models\ApiKey as ChildApiKey;
use models\ApiKeyQuery as ChildApiKeyQuery;
use models\Map\ApiKeyTableMap;

/**
 * Base class that represents a query for the 'api_key' table.
 *
 *
 *
 * @method     ChildApiKeyQuery orderByExpireDate($order = Criteria::ASC) Order by the expire_date column
 * @method     ChildApiKeyQuery orderByName($order = Criteria::ASC) Order by the name column
 * @method     ChildApiKeyQuery orderByKeyValue($order = Criteria::ASC) Order by the key_value column
 ...
 *
 * @method     ChildApiKeyQuery groupByExpireDate() Group by the expire_date column
 * @method     ChildApiKeyQuery groupByName() Group by the name column
 * @method     ChildApiKeyQuery groupByKeyValue() Group by the key_value column
 ...
 *
 * @method     ChildApiKeyQuery leftJoinWith($relation) Adds a LEFT JOIN clause and with to the query
 * @method     ChildApiKeyQuery rightJoinWith($relation) Adds a RIGHT JOIN clause and with to the query
 * @method     ChildApiKeyQuery innerJoinWith($relation) Adds a INNER JOIN clause and with to the query
 *
 ...
 *
 * @method     ChildApiKey findOneByExpireDate(string $expire_date) Return the first ChildApiKey filtered by the expire_date column
 * @method     ChildApiKey findOneByName(string $name) Return the first ChildApiKey filtered by the name column
 * @method     ChildApiKey findOneByKeyValue(string $key_value) Return the first ChildApiKey filtered by the key_value column
 ...
 *
 * @method     ChildApiKey requireOneByExpireDate(string $expire_date) Return the first ChildApiKey filtered by the expire_date column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildApiKey requireOneByName(string $name) Return the first ChildApiKey filtered by the name column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildApiKey requireOneByKeyValue(string $key_value) Return the first ChildApiKey filtered by the key_value column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildApiKey[]|ObjectCollection findByExpireDate(string $expire_date) Return ChildApiKey objects filtered by the expire_date column
 * @method     ChildApiKey[]|ObjectCollection findByName(string $name) Return ChildApiKey objects filtered by the name column
 * @method     ChildApiKey[]|ObjectCollection findByKeyValue(string $key_value) Return ChildApiKey objects filtered by the key_value column
...
abstract class ApiKeyQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \models\Base\ApiKeyQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'phpboilerplate', $modelName = '\\models\\ApiKey', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = ApiKeyTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    {
        $sql = 'SELECT expire_date, name, key_value, created_at, updated_at FROM api_key WHERE key_value = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj = new ChildApiKey();
            $obj->hydrate($row);
            ApiKeyTableMap::addInstanceToPool($obj, null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key);
        }
        $stmt->closeCursor();
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        return $this->addUsingAlias(ApiKeyTableMap::COL_KEY_VALUE, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(ApiKeyTableMap::COL_KEY_VALUE, $keys, Criteria::IN);
    }
    ...
    public function filterByExpireDate($expireDate = null, $comparison = null)
    {
        if (is_array($expireDate)) {
            ...
            if (isset($expireDate['min'])) {
                $this->addUsingAlias(ApiKeyTableMap::COL_EXPIRE_DATE, $expireDate['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($expireDate['max'])) {
                $this->addUsingAlias(ApiKeyTableMap::COL_EXPIRE_DATE, $expireDate['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(ApiKeyTableMap::COL_EXPIRE_DATE, $expireDate, $comparison);
    }
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(ApiKeyTableMap::COL_NAME, $name, $comparison);
    }

    /**
     * Filter the query on the key_value column
     *
     * Example usage:
     * <code>
     * $query->filterByKeyValue('fooValue');   // WHERE key_value = 'fooValue'
     * $query->filterByKeyValue('%fooValue%', Criteria::LIKE); // WHERE key_value LIKE '%fooValue%'
     * </code>
     *
     * @param     string $keyValue The value to use as filter.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildApiKeyQuery The current query, for fluid interface
     */
    public function filterByKeyValue($keyValue = null, $comparison = null)
    {
        if (null === $comparison) {
        ...
        return $this->addUsingAlias(ApiKeyTableMap::COL_KEY_VALUE, $keyValue, $comparison);
    }
    ...
    {
        if ($apiKey) {
            $this->addUsingAlias(ApiKeyTableMap::COL_KEY_VALUE, $apiKey->getKeyValue(), Criteria::NOT_EQUAL);
        }
